...
int BuildSDHummingModel::ReadFromNotes(vector<MIDIINFO> myNotes){
	...
	for(i=0;i<TotalNoteNumber-1;i++){
		myNotes[i].Durationms=myNotes[i+1].Startms-myNotes[i].Startms;
	}
	myNotes[TotalNoteNumber-2].Durationms=myNotes[i].Durationms;
	...
	for (i=0;i<TotalNoteNumber-1;i++)
	{
		printf("  iDurationiWithRest:%d   Durationms:%d\n",pMidiNoteStruct[i].iDurationWithRest,pMidiNoteStruct[i].iDurationms);
	}
...
vector<MIDIINFO> BuildSDHummingModel::LoadMidiFile(char* szInFile){   //得到note的信息，并且将MIDI中可能存在的开始处的长段静音的影响去掉了
...
int BuildSDHummingModel::Write2Model(){
	...
	for (Iter=m_FileTrackmap.begin();Iter!=m_FileTrackmap.end();++Iter){
		vector<MIDIINFO> myNotes=LoadMidiFile((char*)Iter->second.MidiFilename.c_str());  //读取MIDI文件到myNotes
		if(myNotes.size()==0){ 
			printf("file cannot be opened: %s, please check\n",(char*)Iter->second.MidiFilename.c_str());
			return -1;
		}
		///lc
		printf("%s\n",Iter->second.MetaInfo.c_str());
...
int BuildSDHummingModel::writeExtractedTrack(char *OutputModelFileName)   //将提取到的MIDI信息写入文件中
{
	int i;
	int count_iter=0;
	...
	for(VecIter=m_QBHModel.begin();VecIter!=m_QBHModel.end();++VecIter){
		...
		for(i=0;i<VecIter->notes.size();i++){  //pitch 和duration写进文件中
...
bool BuildSDHummingModel::ExtractTrackNo(int TrackNo,string MetaInfo,char* filename){
	...
	if(tag==0){
		m_SongNameVector.push_back(SName);   //将sname存入成员变量中
		...
		for(i=0;i<CountTrackNoteNum;i++){
			NOTE_TEMPLATE Note;    //将note信息存入成员变量中
			...
			myQBHModel.notes.push_back(Note);  //最终存入myQBHModel，每个note的pitch和duration信息
