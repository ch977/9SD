...
void SNormSongNote(SNote *Notes, int iNoteSize,int iLen){
    ...
    for (i=0;i<iNoteSize;i++) 
		Notes[i].fNoteValue=Notes[i].fNoteValue-fMeanNotesValue+NOTE_NORMALIZE_VALUE;   //减平均值，normalizing
}
...
void SPreprocessMelodyDB(float* fPitchArray, int& iLen, int SourceLen,float Ratio){
	...
	for (j=0;j<iLen;j++){
		fPitchArray[j]=fPitchArray[j] - fTargetAverage + PITCH_NORMALIZE_VALUE;  //减去平均值，normalization 
	}
}
