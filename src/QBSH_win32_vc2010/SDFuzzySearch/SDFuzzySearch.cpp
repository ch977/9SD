	...
	/* calculate the note-based similarity against on every song's phrase */
  	for (i=0; i<nModels; i++){
		...
		for (int k=0; k<SQBHModels[i].nPhraseNum; k++){
			...
			int DestLen=SGetNotesPitchLen(SQBHModels[i].sNotes,SQBHModels[i].PhrasePos[k],NotesSize);   //note sequence 的总帧数
...
*/
	...
	for (int n=0; n<nCandidate; n++){
		...
		int NotesSize=SQBHModels[i].nNoteNum-SQBHModels[i].PhrasePos[j];   //从phrase开头到歌曲结尾的note数目 
		int nDesLen=SGetNotesPitchLen(SQBHModels[i].sNotes,SQBHModels[i].PhrasePos[j],NotesSize); //要匹配的模板的phrases所对应的帧数
		...
		for (k=0;k<NotesSize;k++){//对phrases的每一个note
			float fVal=(float)((SQBHModels[i].sNotes[SQBHModels[i].PhrasePos[j]+k].fNoteValue-69.0f)/12.0+log(440.0f)/log(2.0f));  //将pitch 转为Hz
			int NoteDur=(int)(SQBHModels[i].sNotes[SQBHModels[i].PhrasePos[j]+k].fNoteDuration);  //第k个note对应的帧数
			for (m=0;m<NoteDur/myEMDResStru[n].fScale;m++){
				 fDesDataBuf[nFar+m]=fVal;     	//把sequence scale到最佳的长度
			}
		...
		/* binary search in vertical direction for finding the best aligned parameters for key shifting */  //针对基调移位问题，用几个不同的DTW变种
		fSpan = 0.4f;
		fCenter = 0.0;
		int dtw_tag=1;		
 		while(fSpan>0.01){   //递归地，类似于KTRA
		...
		res[n].fScore=fModelMin*FUSION_RATIO+myEMDResStru[n].fScore*(1-FUSION_RATIO);    //note-based 相似度和frame-based 相似度融合起来得到最终结果
		res[n].nModelID=i;
