...
void CplxMul (Complex* spCplxDest, double dSrc)  //real * complex
{    
	spCplxDest->R *= dSrc;
	spCplxDest->I *= dSrc;
}

void Mul (Complex* spCplxDest, double dSrc, long lCount)//real*complex vector
{
	long lLoopCntr;
...
void CplxMul (Complex* spCplxDest, const Complex* spCplxSrc1,
	const Complex* spCplxSrc2){    //two complex multiply
		spCplxDest->R =
...
void Mul (Complex* spCplxDest, const Complex* spCplxSrc1,
		  const Complex* spCplxSrc2, long lCount){   //complex array multiply
	long lLoopCntr;
...
Complex ProdC(Complex Src1, Complex Src2){   //Src1*Src2 ,ProdC=product
	Complex dest;
	Mul(&dest, &Src1, &Src2, 1);
	return dest;
}

Complex* ProdC(Complex* Src1, int nLen,  double Src2){   //real *complex
	long i;
...
void ProdC(Complex* Src1, int nLen,  double Src2, Complex* dst){     //real *complex
	long i;
...
void CplxDiv (Complex* spCplxDest, const Complex* spCplxSrc1,
			  const Complex* spCplxSrc2)  //complex division
{    
	spCplxDest->R =
		(spCplxSrc1->R * spCplxSrc2->R + spCplxSrc1->I * spCplxSrc2->I) /
...
void Div (Complex* spCplxDest, const Complex* spCplxSrc1,
		  const Complex* spCplxSrc2, long lCount)   //complex array division
{    
	long lLoopCntr;
...
Complex DivC(Complex Src1, Complex Src2){ // complex/complex
	Complex dest;
...
Complex DivC(double Src1, Complex Src2)  // double / complex
{     
	Complex dest, temp;
	temp.R = Src1;
	temp.I = 0;
...
Complex ProdC(Complex Src1, double Src2){ //double *complex
	Complex dest = Src1;
	Mul(&dest, Src2, 1);
...
void CplxDiv (Complex* spCplxDest, const Complex* spCplxSrc){  //complex division
	double dReal;
	double dImag;
...
//nLen=5,N=3
{
	...
	for (k = 0; k < N; k++){		
		...
		R[k][k] = (float)sqrt(fSum);  //sqrt of square sum
		for( x=0;x<nPoints;x++){
			Q[k][x] /= R[k][k];   //normalize
		}
...
//parabolic interpolation,to find a more accurate minimum position
{
	const int nPoints = 5;
	const int orden = 2;
	int j;
	...
	km = -x[1]/(2*x[2]);   //accurate position
	di = (float)(x[2]*pow(km,2)+x[1]*km+x[0]);    //parabolic function value(minimum) 
...
//pitch value
{
	...
	fThreshold = VoiceThreshold + m_pendiente*(indMin+1);//min 
	...
	if ((min > fThreshold) || (indMin > (nIndexMax-3))){
		...
		if(minPos<0)
			...
			f0=fs/minPos;   //fundamental frequency
	}
	return f0;
}
...
int Getbilinear(Complex* &npl, Complex* &nzr, double &ngn, 
	...
	Complex c2 = BilFunc1(zc, ns, numtbil, dentbil,vect2);
 	 ...
 	 Complex ngnaux;
	 if (ns == 0){
		 ngnaux = DivC(1.f, c1);  //  1.f/c1  
	 }else{
		 ngnaux = DivC(c2, c1);   //c2/c1
	 }
...
void SCalcDifFunction(const float *fData,int nFrm, int nIndexMax, float fPend,float *oDataBuf){
	...
	for ( int i = 1; i <= nIndexMax; i++ ){
		...
		for(int j=0;j<nFrm+1;j++){
			dif+=(fData[j]-fData[i+j])*(fData[j]-fData[i+j]);	//magnitude difference's square 
		}
		sumd += dif;
