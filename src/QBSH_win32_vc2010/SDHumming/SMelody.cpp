...
int SPitchExtraction(SWaveDataStru mySWaveDataStru, float pendiente, float fmin, float fmax, 
	...
	nFrmWinLen = (int)floor(fs*FRAME_LEN_MS/1000.0f)-1; //window length's frames
	nFrmShtLen = (int)floor(fs*FRAME_SHT_MS/1000.0f);//shift frames
	nIndexMax = (int)ceil(fs/fmin);
	nIndexMin = (int)floor(fs/fmax); //max frequency's index
...
int WaveRead(char* filename, SWaveDataStru &mySWaveDataStru){
	...
	for(int i=0;i<nLen;i++){
		mySWaveDataStru.fDataBuf[i]=pBuffer[i]/32768.0f;    //normalize ,(-1,1)
	}
...
void SProcessQuery(float* fPitchArray,int& Len){
	...
	}//every 5 points get a mean,reduce the dimension,lc
	...
	float temppitch=0;
 	int vadCount=0;
	int nStartTag=0;
	int nStart=0;   //index that start having value bigger than 2(lc)
	for (i=0;i<Len;i++){ 
		if(nStartTag==0){
			if(fPitchArray[i]<2)
				nStart=i;
	...
	//remove the silence frames in the begin(lc)
	{
	...
	if(VadSize<1){
		...
		me=MeanVal/VadSize;  //mean pitch,lc
...
/* note transcription, the input is pitch vector, and the note sequence is generated */
